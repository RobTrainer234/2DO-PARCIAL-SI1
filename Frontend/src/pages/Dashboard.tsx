...
export default function Dashboard() {
  ...
  const cargarIndicadores = async () => {
    try {
      ...
      // Si viene un objeto con 'data', usarlo; si no, usar la respuesta directa
      const datos = response.data?.data || response.data || []
      
      // Asegurar que sea un array
      const listaIndicadores = Array.isArray(datos) ? datos : []

      // Si está vacío, mostrar indicadores por defecto
      ...
      // Mostrar indicadores por defecto en caso de error
      setIndicadores([
        { titulo: 'Total Docentes', valor: 0, unidad: 'docentes', color: '#3498db', icono: '👨‍🏫' },
        { titulo: 'Materias Activas', valor: 0, unidad: 'materias', color: '#2ecc71', icono: '📚' },
        { titulo: 'Grupos', valor: 0, unidad: 'grupos', color: '#e74c3c', icono: '👥' },
      ...
      setError('No se pudieron cargar los indicadores. Se mostrarán valores por defecto.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[
            { titulo: 'Gestión de Usuarios', icono: '👤', desc: 'Administrar usuarios del sistema' },
            { titulo: 'Docentes', icono: '👨‍🏫', desc: 'Registrar y editar docentes' },
            { titulo: 'Materias', icono: '📚', desc: 'Gestionar materias por semestre' },
            { titulo: 'Grupos', icono: '👥', desc: 'Crear y editar grupos' },
            { titulo: 'Aulas', icono: '🏛️', desc: 'Infraestructura disponible' },
            { titulo: 'Horarios', icono: '⏰', desc: 'Configurar horarios y carga docente' }
          ].map((acceso, idx) => (
            <div
              key={idx}
              style={{
